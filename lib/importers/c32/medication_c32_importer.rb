class MedicationC32Importer 
  ...
  def self.entry_xpath
    "cda:entry/cda:substanceAdministration[cda:templateId/@root='2.16.840.1.113883.3.88.11.32.9']"
  end
  
  def self.import_entry(entry_element)
    medication = Medication.new
    with_element(entry_element) do |element|
      coded_name = element.find_first("cda:consumable/cda:manufacturedMaterial/cda:code/@displayName").try(:value)
      if coded_name
        medication.product_coded_display_name = coded_name
      end
      
      product_code = element.find_first("cda:consumable/cda:manufacturedMaterial/cda:code/@code").try(:value)
      if product_code
        medication.product_code = product_code
      end
      
      code_system_oid = element.find_first("cda:consumable/cda:manufacturedMaterial/cda:code/@codeSystem").try(:value)
      if code_system_oid
        medication.code_system = CodeSystem.find_by_code(code_system_oid)
      end
      
      free_text_name = element.find_first("cda:consumable/cda:manufacturedMaterial/cda:name").try(:text)
      if coded_name
